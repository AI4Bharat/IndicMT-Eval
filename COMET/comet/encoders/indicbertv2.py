...
import torch
from transformers import BertConfig, AutoTokenizer, AutoModelForMaskedLM,BertForMaskedLM,AutoConfig

from comet.encoders.base import Encoder


class IndicBertEncoder(Encoder):
    """BERT encoder.
    ...
    def __init__(
        ...
        if load_pretrained_weights:
            ...
            #print("1324355845678901234567890",config)
            self.model = AutoModelForMaskedLM.from_pretrained(
                pretrained_model, output_hidden_states=True
            )
            print(self.model)
    ...
    def from_pretrained(
        ...
        """
        return IndicBertEncoder(pretrained_model, load_pretrained_weights)

    def freeze_embeddings(self) -> None:
        """Frezees the embedding layer."""
        #print("yes freeze emb")
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        #print("---------------------------------------------------")
        #print("Inside Indic bertv2",input_ids)
        ...
        ''' 
        last_hidden_states = outputs[0]  # First element of the output tuple contains last_hidden_states
        all_layers = outputs[-1]  # Last element of the output tuple contains all_layers
        print("Inside bert",len(all_layers))

        return {
            "sentemb": last_hidden_states[:, 0],  # Pooling the [CLS] token
            "wordemb": last_hidden_states,
            "all_layers": all_layers,
            "attention_mask": attention_mask,
            }
        '''
        #print("Output indicbertv2",outputs)
        #print(len(outputs))
        last_hidden_states = outputs.hidden_states[-1]  
        all_layers = outputs.hidden_states  
        sentence_embedding = torch.mean(last_hidden_states, dim=1)
        #print("Inside bert",last_hidden_states[:,0])
        #print("Inside bert",last_hidden_states)
        #print("Inside bert",len(all_layers))
        #print("******************************************************************")
        ...
        #print(outputs)
        last_hidden_states, all_layers = outputs.hidden_states[-1], outputs.hidden_states
        return {
            "sentemb": last_hidden_states[:, 0],  # Pooling the [CLS] token
            "wordemb": last_hidden_states,
            "all_layers": all_layers,
